...
let CartsService = class CartsService {
    ...
    productRepo;
    variantRepo;
    constructor(cartRepo, cartItemRepo, productRepo, variantRepo) {
    ...
    async addToCart(tenantId, userId, dto) {
        ...
        if (existingItem) {
            ...
            if (stockQty !== undefined && stockQty !== null && existingItem.quantity > stockQty) {
                throw new BadRequestException(`Only ${stockQty} items available in stock`);
            }
            ...
            if (variant && quantity > variant.stock_quantity) {
                throw new BadRequestException(`Only ${variant.stock_quantity} items available in stock`);
            }
    ...
    async updateCartItem(tenantId, userId, itemId, dto) {
    ...
    async removeCartItem(tenantId, userId, itemId) {
        const cart = await this.cartRepo.findOne({
            where: { tenant_id: tenantId, user_id: userId },
        });
        if (!cart) {
...
CartsService = __decorate([
    ...
    __param(1, InjectRepository(CartItem)),
    __param(2, InjectRepository(Product)),
    __param(3, InjectRepository(ProductVariant)),
